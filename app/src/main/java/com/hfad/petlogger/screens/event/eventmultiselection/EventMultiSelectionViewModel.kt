...
class EventMultiSelectionViewModel(
    ...
    getAssociatedEvents: GetItemsUseCase<EventForList>? = null,
    getAssociatedNewEvents: GetItemsUseCase<EventForList>? = null
) : ViewModel() {
    val selectionTracker = MultiSelectionTracker<EventForList>(
        allOptionsFetcher = getAllEvents,
        initialSelectionFetcher = getAssociatedEvents,
        initialNewSelectionFetcher = getAssociatedNewEvents,
        ...
        : ViewModelProvider.Factory = object : ViewModelProvider.Factory {
            override fun <T : ViewModel> create(modelClass: Class<T>): T {
                if (modelClass.isAssignableFrom(EventMultiSelectionViewModel::class.java)) {
                    return EventMultiSelectionViewModel(getAllEvents, getAssociatedEvents, getAssociatedNewEvents) as T
                }
