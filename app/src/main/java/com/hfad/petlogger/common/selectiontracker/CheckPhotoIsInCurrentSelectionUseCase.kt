...
class CheckPhotoIsInCurrentSelectionUseCase: CheckItemIsInSelectionUseCase<Photo> {
    ...
    override fun resetToItemList(newList: List<Photo>) {
        ...
        photosInSelection.putAll(newList.map { CheckableItem(it, MutableLiveData(false)) }.associateBy { it.item.id })
    }

    override fun addItems(newItems: List<Photo>) {
        photosInSelection.putAll(newItems.map { CheckableItem(it, MutableLiveData(false)) }.associateBy { it.item.id })
    }

    override fun addItem(newItem: Photo) {
        photosInSelection.putAll(mapOf(Pair(newItem.id, CheckableItem(newItem, MutableLiveData(false)))))
    }
    ...
    override fun addCheckableItems(newItems: List<CheckableItem<Photo>>) {
        photosInSelection.putAll(newItems.onEach { it.isChecked.value = false }.associateBy { it.item.id })
    }
